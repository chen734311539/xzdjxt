<template>
    <div class="userTransferPanel">
       <div class="left">
         <div class="left-top">
           <InputSearch class="seachbar_InputSearch" @on-click="inputSearch" :showClose="true" :changeShowClose="true" 
           @on-change="inputSearchChange" @on-clear="inputSearchClear"/>
        </div>
    ...
    --><div class="right">
        ...
        <div class="right-top" v-show="selectUsers.length>0" style="cursor: pointer;" title="清除已选择的全部人员" @click="clearGroupusers">清除所有</div>
...
import UserTransferItem from '@/components/base/UserTransferItem'


export default {
  name: 'UserTransfer',
  components: {InputSearch,UserTransferItem},
  ...
  methods:{
      ...
      clearGroupusers(){
        ...
        var list = this.$refs.userTransferTree.getCheckedNodes();
        for(var i=0;i<list.length;i++){
          this.userMap[list[i].usercode].checked=false;
          //部门也去掉选择,若部门有多个用户，不会影响其他的已选择用户，不去则不会对此用户产生影响
          var deptcode = this.userMap[list[i].usercode].deptcode;
          if(this.deptMap[deptcode]){
            this.deptMap[deptcode].checked=false;
          }
        }
