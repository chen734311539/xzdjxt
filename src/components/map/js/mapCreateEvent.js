...
export default{
    registerLayerEvent(layerid,layer,mapObj){
        this.registerLayerClickEvent(layerid,layer,mapObj);
        this.registerLayerHoverEvent(layer);
    },
    registerLayerClickEvent(layerid,layer,mapObj) {
        dojo.connect(layer, "onClick", function(evt){
            var needHide = true;
            if(evt.graphic.geometry.type=="polygon"){//面，并且在编辑的时候不隐藏，修改画面时
                 var obj  = mapObj.editToolbar.getCurrentState();
                 if(obj.graphic){//有正在修改的，不隐藏
                    needHide = false;
                 }
            }
            if(needHide){
                markCreateUtil.hideLayer(mapObj.id+"_graphics",mapObj);//编辑图层隐藏
            }
            //隐藏IndoWindow
            mapObj.map.infoWindow.hide();
            var graphic = evt.graphic;
            markCreateUtil.showInfoWindow(graphic,true,mapObj);
        });
    },
