            ...
            <template #titlebutton="{data}">
                ...
                <Poptip v-model="showPop" zIndex="2000" placement="bottom" class="item_option_btn">
                    ...
                    <div slot="content" style="calc(100% - 10px);color:#2d8cf0;min-width:60px">
                        <div v-show="data.istop=='0'" class="itemoption" @click="setItemTop(1)">设置置顶</div>
                        <div v-show="data.istop=='1'" class="itemoption" @click="setItemTop(0)">取消置顶</div>
                        <div class="itemoption" @click="deleteItem">删除主题</div>
                        <div v-show="data.state!='1'" class="itemoption" @click="updateItemstate(1)">初查已完成</div>
                        <div v-show="data.state!='2'" class="itemoption" @click="updateItemstate(2)">在侦案件</div>
                        <div v-show="data.state!='3'" class="itemoption" @click="updateItemstate(3)">已破案件</div>
                    </div>
                  </Poptip>
            </template>
            <template #editfollowpanel="{data}">
                <div class="addueditor">
                    <UE :defaultMsg="defaultMsg" :config="config" ref="ue" ueid="ajzcaddfollow"></UE>
                </div>
...
export default {
  ...
  methods:{
      initEditValues(val){//初始化修改的值
        ...
        paramObj.itemid = val;
        this.http.get("/qzlq/getAjzcItemDetailById",paramObj,this.getAjzcItemDetailByIdCallBack,this,false);
      },
      reflashItem(val){//刷新详情页
        var paramObj = {};
      ...
      getAjzcItemDetailByIdCallBack(result){
      ...
      submitEditContent(){//发表评论
        ...
        if(this.selectusers.length==0){
            this.http.post("/qzlq/insertAjzcItemContent",paramObj,this.insertAjzcItemContentCallBack,this,false);
        }else{
            ...
            this.$msgbox({
              ...
              beforeClose: (action, instance, done) => {
                if (action === 'confirm') {
                  ...
                  paramObj.msg = "案件侦查["+this.itemdata.title+"]主题下添加了关于您的新的信息，请注意查看！";
                  this.http.post("/qzlq/insertAjzcItemContent",paramObj,this.insertAjzcItemContentCallBack,this,false);
                } else {
                  done();
                }
      ...
      deleteReply(val){
          ...
          this.http.get("/qzlq/deleteAjzcItemContent",paramObj,this.deleteAjzcItemContentCallBack,this,false);
      },
      deleteAjzcItemContentCallBack(reslut){
          this.reflashItem(this.itemdata.id);
      },
      editReply(val){
          var obj=JSON.parse(val);
      ...
      replaysubmitEditContent(){
          ...
          this.http.post("/qzlq/updateAjzcItemContent",paramObj,this.updateAjzcItemContentCallBack,this,false);
      },
      updateAjzcItemContentCallBack(result){
          this.showsubmitbtn = "1";
          this.reflashItem(this.itemdata.id);
          this.$refs.ue.setUEContent("");//重置编辑器
      },
      ...
      setItemTop(val){
          ...
          paramObj.itemid = this.itemdata.id;
          if(val==1){
              this.http.get("/qzlq/updateAjzcItemTop",paramObj,this.updateAjzcItemCallBack,this,false);
          }else{
              this.http.get("/qzlq/cancelAjzcItemTop",paramObj,this.updateAjzcItemCallBack,this,false);
          }
      },
      setItemFollow(val){
          var paramObj = {};
          paramObj.itemid = this.itemdata.id;
          if(val==1){
              this.http.get("/qzlq/updateAjzcItemFollow",paramObj,this.updateAjzcItemCallBack,this,false);
          }else{
              this.http.get("/qzlq/cancelAjzcItemFollow",paramObj,this.updateAjzcItemCallBack,this,false);
          }
      },
      deleteItem(){
          var paramObj = {};
          paramObj.itemid = this.itemdata.id;
          this.http.get("/qzlq/deleteAjzcItem",paramObj,this.deleteAjzcItemCallBack,this,false);
      },
      deleteAjzcItemCallBack(){
          this.itemdata = {};
          this.contentlist = [];
          this.page = 1;
          this.totalSize = 0;
      ...
      updateItemstate(val){
          ...
          paramObj.itemid = this.itemdata.id;
          paramObj.state = val;
          this.http.get("/qzlq/updateAjzcItemState",paramObj,this.updateAjzcItemCallBack,this,false);
      },
      updateAjzcItemCallBack(result){
          this.showPop = false;
          this.reflashItem(this.itemdata.id);
      },
      pageChange(page){
...
.ajzcfollowpanel >>> .edui-default .edui-editor {
