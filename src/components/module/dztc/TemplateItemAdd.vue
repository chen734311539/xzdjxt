<template>
    <div class="TemplateItemAdd">
        <div class="flexr" style="justify-content:flex-start;">
            <span>名称：</span><input class="inputClass" v-model="templatename"/>
            <select v-model="selectTemplateid" style="width:132px;margin-left:10px;" class="inputClass" @on-change="changeTemplate">
                <option value="">模板选择</option>
                <option v-for="item in publicTemplateList" :value="item.templateid" :key="item.templateid">{{ item.templatename }}</option>
            </select>
        </div>
        ...
        <div v-show="styletype=='1'&&type=='1'"  class="contentpanel" >
            <div class="flexr" style="justify-content:flex-start;" >
                <div class="biaojipanel flexr" :class="{pointtypeclass:(pointtype=='1')}" @click="clickPointType('1')">纯色标记</div>
                <div class="biaojipanel flexr" style="margin-left:10px;" :class="{pointtypeclass:(pointtype=='2')}" @click="clickPointType('2')">图片标记</div>
            </div>
            ...
            <div v-show="pointtype=='2'" style="margin-top:10px;margin-left:5px;">
                <div class="flexr" style="justify-content:flex-start;">
                    ...
                    <Poptip placement="right" width="300" class="selectMarkIconPop" v-model="showPoptip">
                        <div style="margin-right:10px;margin-left:10px;border:1px solid #b6f6ff;padding:0px 5px;cursor:pointer;">选择</div>
                    ...
                    <Upload :action="insertIconUrl" accept=".png,.bmp,.jpg,.gif,.jpeg" :show-upload-list="false" :on-success="uploadPictureSuccess" :on-error="uploadPictureError">
                        <!-- <Button size="small" type="primary" style="width:80px;">上传新图标</Button> -->
                        <HfxButton  name="上传新图标" :width=80 />
                    </Upload>
                </div>
        ...
        <div class="flexr" style="justify-content:space-evenly;margin-top:10px;">
            <HfxButton  @click="cancelTemplate" name="取消" />
            <HfxButton  @click="insertTemplate" name="保存" />
        </div>
    </div>
</template>
...
export default {
  ...
  methods:{
      getPublicTemplatesCallBack(data){
          this.publicTemplateList = data;
      },
      ...
      insertTemplate(){
        ...
        paramObj.layerid = this.layerobj.layerid;
      ...
      saveTemplateCallBack(data){
          ...
          this.http.get("/tczy/getPublicTemplates",null,this.getPublicTemplatesCallBack,this,false);
          this.$emit('on-save');	
      },
      ...
      getTemplateByIdCallBack(data){
          ...
          for(var i = 0;  i<data.fields.length;i++){
              ...
              if(map[row]){
                 map[row].collist.push({col:col,name:name});
              }else{
                  var rowobj = {};
                  rowobj.collist=[];
                  rowobj.row = row;
                  rowobj.collist.push({col:col,name:name});
                  this.moduleList.push(rowobj);
                  map[row]=rowobj;
              }
          }
...
/* style="border-top:1px solid white;margin-top:10px;padding:10px;background:#464646;" */
.contentpanel{
    margin-top: 10px;
    padding: 10px;
