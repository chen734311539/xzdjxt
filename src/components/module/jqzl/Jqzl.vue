<template>
    <div class="jqzlpanel">
      <LeftContentNameBar :isshowshaixuan="true" :isshowclear="isshowclear" @on-click="clickMoreIcon" @on-clear="fanhuisearch"  />
        <JqzlAddInfo :show="showAddObj.showmodal" :showdata="showAddObj" @handler="addItemHandler"/>
        <JqzlEditInfo :show="showEditObj.showmodal"  :showdata="showEditObj" ref="jqzledit"/>
        <div class="seachbar">
            ...
            <div class="seachbar_caozuo">
              ...
              <div class="seachbar_add" @click="showAddJqzlItem" title="添加" ></div>
            </div>
        </div>
        <div class="jqzllist-panel">
            <JqzlItemButton v-for="jqzlObj in jqzlItemList" :key="jqzlObj.id" :data="jqzlObj" @show-edit="showEditPanel(jqzlObj)" />
        </div>
        <PagerNew class="pagebar" :totalSize="totalSize" :pageSize="pageSize" @on-change="pageChange" :nowPage="page"/>

        <Modal v-model="showMore" :draggable = "true" title="警情质量筛选" class-name="jqzlselect-panel">
...
import JqzlAddInfo from '@/components/module/jqzl/JqzlAddInfo'
import JqzlEditInfo from '@/components/module/jqzl/JqzlEditInfo'
import JqzlItemButton from '@/components/module/jqzl/JqzlItemButton'
...
export default {
  ...
  components: { InputSearchNew,DropdownTreeNew,PagerNew,JqzlAddInfo,JqzlEditInfo,JqzlItemButton,LeftContentNameBar},
  data () {
    return {
      deptListObj:{},
      showMore:false,
  ...
  methods:{
      addItemHandler(data,state){//编辑Item页面返回
        if(state=="reflash"){
          this.page = 1;
          this.searchJqzlItems();
        }
      },
      showEditPanel(val){
        this.$refs.jqzledit.data = val;
        this.$refs.jqzledit.initEditValues();
      ...
      searchJqzlItems(){
        ...
        this.http.get("/qzlq/getJqzlItems",paramObj,this.getJqzlItemsCallBack,this,false);
      },
      getJqzlItemsCallBack(data){
        this.totalSize = data.count;
        this.jqzlItemList = data.list;
      },
      getJqzlItemsFirstCallBack(data){
        this.totalSize = data.count;
        this.jqzlItemList = data.list;
        if(this.jqzlItemList.length>0){
          this.initRightPaneldata(this.jqzlItemList[0].id,"jqzl");
        }
      }
  },
  created(){
     ...
     paramObj.page = this.page;
     paramObj.pagesize = this.pageSize;
     this.http.get("/qzlq/getJqzlItems",paramObj,this.getJqzlItemsFirstCallBack,this,false);
  }
}
</script>
...
<style>
.jqzlselect-panel .ivu-modal .ivu-modal-content{
  top: 82px;
  left: 387px;
