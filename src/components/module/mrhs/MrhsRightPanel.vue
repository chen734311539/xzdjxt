<template>
    <div class="mrhsRightPanel">
        <LeftContentNameBar :isshowshaixuan="false"  style="margin:0 0 0 0;" />
        <div v-if="contentlist.length>0"  class="mrhsfollowpanel">
        <HfxFollowTemplate  :itemdata="itemdata" :contentlist="contentlist" @on-commentreply="commentReply" @on-editreply="editReply" @on-deletereply="deleteReply" >
            <template #titlebutton="{data}">
                <HfxButton @click="downMrhsZld" name="下载文档" :width=70  />
            </template>
            <template #editfollowpanel="{data}">
                <div class="addueditor">
                    <UE :defaultMsg="defaultMsg" :config="config" ref="ue" ueid="mrhsaddfollow"></UE>
                </div>
  ...
  methods:{
      ...
      submitEditContent(){//发表评论
        ...
        if(this.selectusers.length==0){
            this.http.post("/qzlq/insertMrhsItemContent",paramObj,this.insertMrhsItemContentCallBack,this,false);
        }else{
            ...
            this.$msgbox({
              ...
              beforeClose: (action, instance, done) => {
                if (action === 'confirm') {
                  ...
                  paramObj.msg = "每日会商["+this.itemdata.ymd+"]主题下添加了关于您的新的信息，请注意查看！";
                  this.http.post("/qzlq/insertMrhsItemContent",paramObj,this.insertMrhsItemContentCallBack,this,false);
                } else {
                  done();
                }
      ...
      deleteReply(val){
          ...
          this.http.get("/qzlq/deleteMrhsItemContent",paramObj,this.deleteMrhsItemContentCallBack,this,false);
      },
      deleteMrhsItemContentCallBack(reslut){
          this.reflashItem(this.itemdata.id);
      },
      editReply(val){
          var obj=JSON.parse(val);
      ...
      replaysubmitEditContent(){
          ...
          this.http.post("/qzlq/updateMrhsItemContent",paramObj,this.updateMrhsItemContentCallBack,this,false);
      },
      updateMrhsItemContentCallBack(result){
          this.showsubmitbtn = "1";
          this.reflashItem(this.itemdata.id);
          this.$refs.ue.setUEContent("");//重置编辑器
      },
      ...
      signedZld(){
          var paramObj = {};
          paramObj.itemid = this.itemdata.id;
          this.http.get("/qzlq/signedMrhsItem",paramObj,this.updateMrhsItemCallBack,this,false);
      },
      setItemTop(val){
          var paramObj = {};
          paramObj.itemid = this.itemdata.id;
          if(val==1){
              this.http.get("/qzlq/updateMrhsItemTop",paramObj,this.updateMrhsItemCallBack,this,false);
          }else{
              this.http.get("/qzlq/cancelMrhsItemTop",paramObj,this.updateMrhsItemCallBack,this,false);
          }
      },
      setItemFollow(val){
          var paramObj = {};
          paramObj.itemid = this.itemdata.id;
          if(val==1){
              this.http.get("/qzlq/updateMrhsItemFollow",paramObj,this.updateMrhsItemCallBack,this,false);
          }else{
              this.http.get("/qzlq/cancelMrhsItemFollow",paramObj,this.updateMrhsItemCallBack,this,false);
          }
      },
      deleteItem(){
          var paramObj = {};
      ...
      updateItemstate(val){
          ...
          paramObj.itemid = this.itemdata.id;
          paramObj.state = val;
          this.http.get("/qzlq/updateMrhsItemState",paramObj,this.updateMrhsItemCallBack,this,false);
      },
...
.mrhsfollowpanel >>> .titlebutton {
    margin: 0 0 0 0;
}
.mrhsfollowpanel >>> .edui-default .edui-editor {
