<template>
    <div class="swxxpanel">
      <LeftContentNameBar :isshowshaixuan="true" :isshowclear="isshowclear" @on-click="clickMoreIcon" @on-clear="fanhuisearch"  />
        <SwxxAddInfo :show="showAddObj.showmodal" :showdata="showAddObj" @handler="addItemHandler"/>
        <SwxxEditInfo :show="showEditObj.showmodal"  :showdata="showEditObj" ref="swxxedit"/>
        <div class="seachbar">
            ...
            <div class="seachbar_caozuo">
              ...
              <div class="seachbar_add" @click="showAddSwxxItem" title="添加" ></div>
            </div>
        </div>
        <div class="swxxlist-panel">
            <SwxxItemButton v-for="swxxObj in swxxItemList" :key="swxxObj.id" :data="swxxObj" @show-edit="showEditPanel(swxxObj)" />
        </div>
        <PagerNew class="pagebar" :totalSize="totalSize" :pageSize="pageSize" @on-change="pageChange" :nowPage="page"/>

        <Modal v-model="showMore" :draggable = "true" title="涉稳信息筛选" class-name="swxxselect-panel">
...
import SwxxAddInfo from '@/components/module/swxx/SwxxAddInfo'
import SwxxEditInfo from '@/components/module/swxx/SwxxEditInfo'
import SwxxItemButton from '@/components/module/swxx/SwxxItemButton'
...
export default {
  ...
  components: { InputSearchNew,DropdownTreeNew,PagerNew,SwxxAddInfo,SwxxEditInfo,SwxxItemButton,LeftContentNameBar},
  data () {
    return {
      deptListObj:{},
      showMore:false,
  ...
  methods:{
      addItemHandler(data,state){//编辑Item页面返回
        if(state=="reflash"){
          this.page = 1;
          this.searchSwxxItems();
        }
      },
      showEditPanel(val){
        this.$refs.swxxedit.data = val;
        this.$refs.swxxedit.initEditValues();
      ...
      searchSwxxItems(){
        ...
        this.http.get("/qzlq/getSwxxItems",paramObj,this.getSwxxItemsCallBack,this,false);
      },
      getSwxxItemsCallBack(data){
        this.totalSize = data.count;
        this.swxxItemList = data.list;
      },
      getSwxxItemsFirstCallBack(data){
        this.totalSize = data.count;
        this.swxxItemList = data.list;
        if(this.swxxItemList.length>0){
          this.initRightPaneldata(this.swxxItemList[0].id,"swxx");
        }
      }
  },
  created(){
     ...
     paramObj.page = this.page;
     paramObj.pagesize = this.pageSize;
     this.http.get("/qzlq/getSwxxItems",paramObj,this.getSwxxItemsFirstCallBack,this,false);
  }
}
</script>
...
<style>
.swxxselect-panel .ivu-modal .ivu-modal-content{
  top: 82px;
  left: 387px;
