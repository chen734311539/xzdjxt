<template>
    <Modal v-model="showdata.showmodal" :draggable = "true" :footer-hide="true" title="修改涉稳信息主题" class-name="swxxEditItem-panel">
      <div slot="close"><div class="hfx-modal-close"/></div>
        <div style="display:block;">
            <div style="height: 570px;">
                <div class="swxxEditPanel">
                    <div class="modal-div">
                        <span>选择参与部门：</span>
                ...
                <div class="swxxEditContentPanel">
                    <div class="uespanel">
                        <UE :defaultMsg="defaultMsg" :config="config" ref="ue" ueid="swxxedit"></UE>
                    </div>
                </div>
            </div>
            <div class="swxxEditBtn">
                <HfxButton  @click="addSwxxItem" />
                <HfxButton  @click="closeSwxx" name="取消" />
            </div>
        </div>
    </Modal>
</template>
...
export default {
  ...
  computed: mapState({
    ...
    deptListObj: state => state.swxx.allDeptListObj
  }),
  methods:{
      addSwxxItem(){
          ...
          this.http.post("/qzlq/updateSwxxItem",updateItemParamObj,this.updateSwxxItemCallBack,this,false);
      },
      ...
      getDeptTreeCallBack(data){
         this.$store.dispatch('putChildState',{"key":"swxx","childkey":"allDeptListObj","value":data});
      },
...
.swxxEditItem-panel .ivu-modal .ivu-modal-content{
  top:180px;
  left:495px;
