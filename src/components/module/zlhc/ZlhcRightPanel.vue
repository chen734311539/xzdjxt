<template>
    <div class="zlhcRightPanel">
      <LeftContentNameBar :isshowshaixuan="false"  style="margin:0 0 0 0;" />
      <div v-if="contentlist.length>0"  class="zlhcfollowpanel">
        <HfxFollowTemplate  :itemdata="itemdata" :contentlist="contentlist" @on-commentreply="commentReply" @on-editreply="editReply" @on-deletereply="deleteReply" >
            <template #titlebutton="{data}">
                <HfxButton @click="downZlhcZld" name="下载指令单" :width=100  />
            </template>
            <template #editfollowpanel="{data}">
                <div class="addueditor">
                    <UE :defaultMsg="defaultMsg" :config="config" ref="ue" ueid="zlhcaddfollow"></UE>
                </div>
...
export default {
  ...
  methods:{
      ...
      getZlhcItemDetailByIdCallBack(result){
          ...
          this.zldurl = result.item.wordurl;
          var that = this;
      },
      submitEditContent(){//发表评论
        ...
        if(this.selectusers.length==0){
            this.http.post("/qzlq/insertZlhcItemContent",paramObj,this.insertZlhcItemContentCallBack,this,false);
        }else{
            ...
            this.$msgbox({
              ...
              beforeClose: (action, instance, done) => {
                if (action === 'confirm') {
                  ...
                  paramObj.msg = "指令核查["+this.itemdata.title+"]主题下添加了关于您的新的信息，请注意查看！";
                  this.http.post("/qzlq/insertZlhcItemContent",paramObj,this.insertZlhcItemContentCallBack,this,false);
                } else {
                  done();
                }
      ...
      deleteReply(val){
          ...
          this.http.get("/qzlq/deleteZlhcItemContent",paramObj,this.deleteZlhcItemContentCallBack,this,false);
      },
      deleteZlhcItemContentCallBack(reslut){
          this.reflashItem(this.itemdata.id);
      },
      editReply(val){
          var obj=JSON.parse(val);
      ...
      replaysubmitEditContent(){
          ...
          this.http.post("/qzlq/updateZlhcItemContent",paramObj,this.updateZlhcItemContentCallBack,this,false);
      },
      updateZlhcItemContentCallBack(result){
          this.showsubmitbtn = "1";
      ...
      setItemTop(val){
          ...
          paramObj.itemid = this.itemdata.id;
          if(val==1){
              this.http.get("/qzlq/updateZlhcItemTop",paramObj,this.updateZlhcItemCallBack,this,false);
          }else{
              this.http.get("/qzlq/cancelZlhcItemTop",paramObj,this.updateZlhcItemCallBack,this,false);
          }
      },
      setItemFollow(val){
          var paramObj = {};
          paramObj.itemid = this.itemdata.id;
          if(val==1){
              this.http.get("/qzlq/updateZlhcItemFollow",paramObj,this.updateZlhcItemCallBack,this,false);
          }else{
              this.http.get("/qzlq/cancelZlhcItemFollow",paramObj,this.updateZlhcItemCallBack,this,false);
          }
      },
      deleteItem(){
          var paramObj = {};
      ...
      updateItemstate(val){
          ...
          paramObj.itemid = this.itemdata.id;
          paramObj.state = val;
          this.http.get("/qzlq/updateZlhcItemState",paramObj,this.updateZlhcItemCallBack,this,false);
      },
...
.zlhcfollowpanel >>> .titlebutton {
    margin: 0 0 0 0;
}
.zlhcfollowpanel >>> .edui-default .edui-editor {
