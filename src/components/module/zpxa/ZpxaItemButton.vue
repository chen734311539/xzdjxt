<template>
    <div class="zpxaItemButton" @click="showFollowPanel" @mouseover="mouseOver" @mouseout="mouseOut">
      <div class="zpxabuttonTX" :class="zpxabuttonBorder">
        <img v-show="data.istop=='1'" src="@/assets/jq/ssjq_zhiding.png" class="zhiding"/>
      </div>
        <div class="content">
            <span class="title"  :style="{color:(data.state=='1'?'#494dfa':data.state=='2'?'#fdcf15':data.state=='3'?'#f94f21':data.state=='4'?'#0abf3d':'#fff')}" :title="data.title">{{titlespan}}</span>
            ...
            <div class="caozuo" v-show="showEditIcon" >
              <div class="mdyIcon" @click="editZpxaItem" ></div>
            </div>
         </div>
        <div :class="state">
        </div>
...
export default {
  ...
  data () {
    return {
        ...
        zpxabuttonBorder:"zpxabuttonBorder_leave",
    }
  },
  computed: {
    titlespan:function(){
  ...
  methods:{
    ...
    mouseOver(){
       ...
       this.zpxabuttonBorder="zpxabuttonBorder_enter";
     },
     mouseOut(){
       this.showEditIcon = false;
       this.zpxabuttonBorder="zpxabuttonBorder_leave";
     },
     editZpxaItem(){
         this.$emit("show-edit",true);
     },
     showFollowPanel(){
       this.initRightPaneldata(this.data.id,"zpxa");
     }
  },
  created(){
...
.zpxabuttonTX:hover, .zpxabuttonTX:focus, .zpxabuttonTX:active {
    cursor: pointer;
...
.mdyIcon{
  ...
  margin-right:10px;
  background: 
  url(./../../../assets/zpxa/zpxa_mdy.png)  no-repeat;
}
.state_dfx{
  position: absolute;
  right: 5px;
  ...
  height: 45px;
  background: 
  url(./../../../assets/jqlz/jqlz_dfx.png)  no-repeat;
}
.state_zfx{
  position: absolute;
  right: 5px;
  ...
  height: 45px;
  background: 
  url(./../../../assets/jqlz/jqlz_zfx.png)  no-repeat;
}
.state_gfx{
  position: absolute;
  right: 5px;
  ...
  height: 45px;
  background: 
  url(./../../../assets/jqlz/jqlz_gfx.png)  no-repeat;
}
.state_yhj{
  position: absolute;
  right: 5px;
  ...
  height: 45px;
  background: 
  url(./../../../assets/jqlz/jqlz_yhj.png)  no-repeat;
}
</style>
